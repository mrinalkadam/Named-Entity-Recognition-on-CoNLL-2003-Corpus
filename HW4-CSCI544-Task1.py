...
glove_embeddings_to_be_used = False
cnn_for_character_level_embedding_to_be_used = False
...
def batching(data):
    ...
    char_sequence_tensor = torch.zeros((batch_size, max_length_of_word_sequence, max_length_of_character_sequence), dtype=torch.long)
    ...
    for index in range(batch_size):
        ...
        if batch_data[index].ids_of_tags:
            label_sequence_tensor[index, :length_of_word_sequence[index]] = torch.LongTensor(batch_data[index].ids_of_tags)

        for word_index in range(length_of_word_sequence[index]):
            char_sequence_tensor[index, word_index, :length_of_character_sequence[index, word_index]] = torch.LongTensor(
...
def model_evaluation(model: BiLSTM, data, best_f_score, dataset = "Train"):
    ...
    prediction_file_name = dataset + '1.out'
    score_file_name = dataset + '1_score.out'
...
def train_model(model: BiLSTM, batches_of_train_data, batches_of_dev_data):
    ...
    f_scores = csv.writer(open('f_scores_1.csv', 'w'))
...
if __name__ == "__main__":
    ...
    tag_prediction(model, dev_batches, 'dev1.out')
    tag_prediction(model, test_batches, 'test1.out')
