...
def model_evaluation(model: BiLSTM, data, best_f_score, dataset="Train"):
    ...
    prediction_file_name = dataset + '2.out'
    score_file_name = dataset + '2_score.out'
...
def train_model(model: BiLSTM, batches_of_train_data, batches_of_dev_data):
    ...
    f_scores = csv.writer(open('f_scores_2.csv', 'w'))
...
if __name__ == "__main__":
    ...
    tag_prediction(model, dev_batches, 'dev2.out')
    tag_prediction(model, test_batches, 'test2.out')
